package top.iot.gateway.component.gateway.annotation;
...
/**
 * 从事件总线{@link top.iot.gateway.core.event.EventBus}中订阅消息并执行注解的方法,
 * 事件总线的输出数据可以作为方法参数,如果类型不一致会自动转换。
 * 也可以通过方法参数直接获取事件总线的原始数据:{@link top.iot.gateway.core.event.TopicPayload}
 *
 * <pre>
 ...
 * </pre>
 * @author zhouhao
 * @see top.iot.gateway.core.event.EventBus
 * @see top.iot.gateway.community.gateway.spring.SpringMessageBroker
 * @since 1.0
 */
...
public @interface Subscribe {
     ...
     * @return topics
     * @see Subscribe#value()
     * @see top.iot.gateway.core.event.EventBus#subscribe(Subscription)
     */
    @AliasFor("value")
