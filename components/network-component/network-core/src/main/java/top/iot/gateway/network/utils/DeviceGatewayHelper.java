...
import top.iot.gateway.component.common.PropertyConstants;
...
import top.iot.gateway.core.message.state.DeviceStateCheckMessage;
import top.iot.gateway.core.message.state.DeviceStateCheckMessageReply;
...
public class DeviceGatewayHelper {
    ...
    public Mono<DeviceOperator> handleDeviceMessage(DeviceMessage message,
        ...
        return this
                ...
                .then(then)
                .contextWrite(Context.of(DeviceMessage.class, message));//将消息放入上下文中
        ...
        return sessionManager
                ...
                .map(old -> {
                    ...
                    if (needUpdateSession(old, message)) {
                        ...
                        return sessionManager.compute(deviceId, null, session -> updateSession(session, message, sessionBuilder));
                    }
                ...
                .defaultIfEmpty(Mono.defer(() -> {
                    log.info("[{}]创建或者更新会话", deviceId);
                    return sessionManager.compute(
                            deviceId,
                            createNewSession(deviceId, message, sessionBuilder,
                                    () -> {
                                        //设备注册
                                        if (isDoRegister(message)) {
                                            return messageHandler
                ...
                .flatMap(device -> sessionBuilder
                        ...
                        .map(newSession -> {
                            ...
                            if (message.getHeader(Headers.keepOnline).orElse(false)) {
                                ...
                                log.info("[{}]创建会话成功 {}", deviceId, newSession);
                            }
                            return newSession;
                        }));
    }
    ...
    private Mono<DeviceSession> updateSession0(DeviceSession session,
        ...
        session.keepAlive();
        log.info("[{}]更新会话成功 {}", message.getDeviceId(), session);
        return after == null
                ? Mono.just(session)
    ...
    private static boolean isDoRegister(DeviceMessage message) {
        return message instanceof DeviceRegisterMessage
                && message.getHeader(PropertyConstants.deviceName.getKey()).isPresent()
                && message.getHeader(PropertyConstants.productId.getKey()).isPresent();
    }
