...
create_keys() {
   ...
   keytool -genkeypair -alias root -keyalg EC -dname 'C=CN,L=ChongQing,O=iot-gateway Pro,OU=Iot Platform,CN=root' \
        -ext BC=ca:true -validity $VALIDITY -keypass $TRUST_STORE_PWD -keystore $TRUST_STORE -storepass $TRUST_STORE_PWD

   echo "creating CA key and certificate..."
   keytool -genkeypair -alias ca -keyalg EC -dname 'C=CN,L=ChongQing,O=iot-gateway Pro,OU=iot-gateway Iot Platform,CN=ca' \
        -ext BC=ca:true -validity $VALIDITY -keypass $TRUST_STORE_PWD -keystore $TRUST_STORE -storepass $TRUST_STORE_PWD
   ...
   keytool -genkeypair -alias server -keyalg EC -dname 'C=CN,L=ChongQing,O=iot-gateway Pro,OU=iot-gateway Iot Platform,CN=server' \
   ...
   keytool -genkeypair -alias client -keyalg EC -dname 'C=CN,L=ChongQing,O=iot-gateway Pro,OU=iot-gateway Iot Platform,CN=client' \
        -validity $VALIDITY -keypass $KEY_STORE_PWD -keystore $KEY_STORE -storepass $KEY_STORE_PWD
   ...
   keytool -genkeypair -alias self -keyalg EC -dname 'C=CN,L=ChongQing,O=iot-gateway Pro,OU=iot-gateway Iot Platform,CN=self' \
   ...
   keytool -genkeypair -alias nosigning -keyalg EC -dname 'C=CN,L=ChongQing,O=iot-gateway Pro,OU=iot-gateway Iot Platform,CN=nosigning' \
