...
import top.iot.gateway.core.message.firmware.*;
...
public class TopicMessageCodec {
    ...
    protected ReportPropertyMessage handleGBReportProperty(JSONObject json) {
        ReportPropertyMessage msg = ReportPropertyMessage.create();
        msg.setMessageId(json.getString("eventId"));
        msg.setDeviceId(json.getString("deviceId"));
        msg.setProperties(JSONObject.parseObject(json.getString("properties")));
        msg.setTimestamp(json.getLong("timestamp"));
        return msg;
    }

    protected List<? extends Message> handleGBReportEvent(JSONObject json) {
        JSONObject events = JSONObject.parseObject(json.getString("events"));
    ...
    protected DeviceMessage doDecode(String deviceId, String topic, JSONObject payload) {
        ...
        } else if (topic.endsWith("/report-property")) { //定时上报属性
